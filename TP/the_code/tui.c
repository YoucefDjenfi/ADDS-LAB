...
void show_splash_screen() {
    ...
    typing_animation_centered("  _______ _____  _____ ______    _______ ________   _________ ", 2);
    typing_animation_centered(" |__   __|  __ \\|_   _|  ____|  |__   __|  ____\\ \\ / /__   __|", 2);
    typing_animation_centered("    | |  | |__) | | | | |__        | |  | |__   \\ V /   | |   ", 2);
    typing_animation_centered("    | |  |  _  /  | | |  __|       | |  |  __|   > <    | |   ", 2);
    typing_animation_centered("    | |  | | \\ \\ _| |_| |____      | |  | |____ / . \\   | |   ", 2);
    typing_animation_centered("    |_|  |_|  \\_\\_____|______|     |_|  |______/_/ \\_\\  |_|   ", 2);
    printf("%s", COLOR_RESET);
    printf("\n");
    typing_animation_centered("Premium Text Analysis & Trie Management System", 10);
    printf("\n");
    draw_divider();
...
void display_menu() {
    ...
    printf("  %s[1]%s %-20s %s[2]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Load a file", COLOR_YELLOW, COLOR_LIGHT_BLUE, "List loaded files");
    printf("  %s[3]%s %-20s %s[4]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Set Union", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Set Intersection");
    printf("  %s[5]%s %-20s %s[6]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Set Difference", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Check Similarity");
    printf("  %s[7]%s %-20s %s[8]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Find File Topic", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Prefix Search");
    printf("  %s[9]%s %-20s %s[0]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Count Matches", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Back");
    printf("%s", COLOR_RESET);
...
void set_operation_flow(int type) {
    ...
    switch(type) {
        case 1:
            result = union_a_b(loaded_files[idx1].combined_trie, loaded_files[idx2].combined_trie);
            op_name = "Union";
            break;
        case 2:
            result = intersection(loaded_files[idx1].combined_trie, loaded_files[idx2].combined_trie);
            op_name = "Intersection";
            break;
        case 3:
            result = difference(loaded_files[idx1].combined_trie, loaded_files[idx2].combined_trie);
            op_name = "Difference (A - B)";
            break;
    }
...
void find_topic_flow() {
    ...
    if (t) {
        ...
        printf("  Could not identify a clear topic.\n");
    }
    draw_divider();
    press_any_key();
}
...
void prefix_search_flow() {
    ...
    char **results = words_start_with(loaded_files[idx].combined_trie, prefix);
    if (results) {
...
void prefix_count_flow() {
    ...
    int count = countPrefixMatches(loaded_files[idx].combined_trie, prefix);
    printf("\n  %sStatistics:%s\n", COLOR_LIGHT_BLUE, COLOR_RESET);
    draw_divider();
...
void start_word_tui() {
    ...
    do {
        ...
        switch(choice) {
            case 1: load_file_flow(); break;
            case 2: list_files(); break;
            case 3: set_operation_flow(1); break;
            case 4: set_operation_flow(2); break;
            case 5: set_operation_flow(3); break;
            case 6: similarity_flow(); break;
            case 7: find_topic_flow(); break;
            case 8: prefix_search_flow(); break;
            case 9: prefix_count_flow(); break;
            case 0:
                printf("\n");
...
void show_sentence_file_list() {
    ...
    for (int i = 0; i < sentence_file_count; i++) {
        printf("  %s[%d]%s %s%s%s\n", COLOR_YELLOW, i + 1, COLOR_RESET, COLOR_LIGHT_BLUE, loaded_sentences[i].filename, COLOR_RESET);
    }
    draw_divider();
}
...
void set_operation_sentence_flow(int type) {
    ...
    if (idx1 < 1 || idx1 > sentence_file_count || idx2 < 1 || idx2 > sentence_file_count) {
        printf("  %sInvalid selection.%s\n", COLOR_YELLOW, COLOR_RESET);
        ms_sleep(1000);
        return;
    }
    ...
    char *op_name = "";
    switch(type) {
        case 1: result = union_a_b_S(loaded_sentences[idx1].combined_trie_S, loaded_sentences[idx2].combined_trie_S); op_name = "Union"; break;
        case 2: result = intersection_S(loaded_sentences[idx1].combined_trie_S, loaded_sentences[idx2].combined_trie_S); op_name = "Intersection"; break;
        case 3: result = difference_S(loaded_sentences[idx1].combined_trie_S, loaded_sentences[idx2].combined_trie_S); op_name = "Difference"; break;
    }
...
void similarity_sentence_flow() {
    ...
    double sim = calculateJaccard_S(loaded_sentences[idx1].combined_trie_S, loaded_sentences[idx2].combined_trie_S);
...
void prefix_search_sentence_flow() {
    ...
    char **results = sentences_start_with_S(loaded_sentences[idx].combined_trie_S, prefix);
    if (results) {
...
void start_sentence_tui() {
    ...
    do {
        ...
        printf("  %s[1]%s %-20s %s[2]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Load file", COLOR_YELLOW, COLOR_LIGHT_BLUE, "List files");
        printf("  %s[3]%s %-20s %s[4]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Union", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Intersection");
        printf("  %s[5]%s %-20s %s[6]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Difference", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Similarity");
        printf("  %s[7]%s %-20s %s[0]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Prefix Search", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Back");
        printf("%s", COLOR_RESET);
        ...
        switch(choice) {
            case 1: load_sentence_file_flow(); break;
            case 2: list_sentences_files(); break;
            case 3: set_operation_sentence_flow(1); break;
            case 4: set_operation_sentence_flow(2); break;
            case 5: set_operation_sentence_flow(3); break;
            case 6: similarity_sentence_flow(); break;
            case 7: prefix_search_sentence_flow(); break;
            case 0:
                printf("\n");
                print_centered("Returning to mode selection...", COLOR_DIM);
                ms_sleep(500);
...
void start_tui() {
    ...
    do {
        ...
        printf("  %s[2]%s %-20s\n", COLOR_YELLOW, COLOR_LIGHT_BLUE, "Sentence Mode");
    ...
    clear_screen();
    printf("\n");
    print_centered("Thank you for using Trie Text System. Goodbye!", COLOR_LIGHT_BLUE);
    printf("\n");
}
